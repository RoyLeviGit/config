# Generated by Powerlevel10k configuration wizard on 2024-05-04 at 12:46 IDT.
...
GH_USER_CACHE="$HOME/.cache/gh_user"

# Function to update the GitHub user cache.
function update_gh_user_cache() {
  gh api user --jq .login > "$GH_USER_CACHE"
}

# Function for your prompt to read the cached GitHub user name.
function prompt_my_gh_user() {
  # if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    if [ ! -f "$GH_USER_CACHE" ]; then
      update_gh_user_cache
    fi
    local gh_user=$(cat "$GH_USER_CACHE")
    if [[ -n $gh_user ]]; then
      # Check if the GitHub username matches your work username
      if [[ "$gh_user" == "RoyLeviGit" ]]; then
        # Display a different emoji for private username
        p10k segment -f 208 -i '🐙'
      else
        # Display one emoji for the any other username
        p10k segment -f 208 -i '🦄'
      fi
    fi
  # fi
